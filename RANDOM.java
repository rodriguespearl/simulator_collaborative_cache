...
public class RANDOM extends CacheReplacementAlgorithm{
	static int cacheSize;
	Random random;
	final String name = "RANDOM";
	GenerateRequest generateObj;

	public RANDOM(int cacheSize, GenerateRequest generateObj){
	...
	public int remove(ArrayList<ArrayList<Integer>> cache, int curSize) {
		//SimulatorLogger.writeLog(name, "Removing a random block", infoLevel);
		//System.out.println("CACHE ALGO: Removing a random block");
		int position = random.nextInt(cache.size());
		//System.out.println("Cache size " + cache.size());
		//System.out.println("Position " + position);
		ArrayList<Integer> items = cache.remove(position);
